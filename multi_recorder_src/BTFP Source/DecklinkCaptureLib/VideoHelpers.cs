...
namespace DecklinkCaptureLib
{
    ...
    public class VideoHelpers
    {
        ...
        public static IBaseFilter LoadFilter(string libFileName, Guid clsid, ref IntPtr hDllPtr)
        {
            ...
            IBaseFilter filter = (IBaseFilter)Marshal.GetObjectForIUnknown(bfPtr);
            Marshal.Release(bfPtr);
            return filter;
        }
            ...
            return devices.Where(device =>
                {
                    ...
                    IPersist persist;
                    device.Mon.BindToObject(null, null, typeof(IPersist).GUID, out persistObject);
                    persist = (IPersist)persistObject;
                    Guid clsid;
                    persist.GetClassID(out clsid);
                    return (clsid == typeof(DSoundRender).GUID);
