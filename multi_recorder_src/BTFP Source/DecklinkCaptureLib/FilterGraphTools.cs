...
namespace DirectShowLib.Utils
{
    ...
    public sealed class FilterGraphTools
    {
        ...
        public static IBaseFilter AddFilterByName(IGraphBuilder graphBuilder, Guid deviceCategory, string friendlyName, string name = "")
        {
            int hr = 0;
            IBaseFilter filter = null;
            ...
            for(int i = 0; i < devices.Length; i++)
            {
                ...
                hr = (graphBuilder as IFilterGraph2).AddSourceFilterForMoniker(devices[i].Mon, null, name, out filter);
                DsError.ThrowExceptionForHR(hr);
