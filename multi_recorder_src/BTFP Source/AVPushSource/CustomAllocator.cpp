...
CCustomAllocator::CCustomAllocator(TCHAR *pName, LPUNKNOWN pUnk, HRESULT *phr, BOOL bEvent, BOOL fEnableReleaseCallback)
    ...
    , m_pBuffer(NULL)
	, m_lFree(L"Custom allocator free list", SAMPLE_LIST_SIZE)
	, m_lDeliver(L"Decklink push source custom allocator deliver list", SAMPLE_LIST_SIZE)
	, m_lBuffers(L"Decklink push source custom allocator buffer list", SAMPLE_LIST_SIZE)
	, m_hSemFree(NULL)
	, m_lWaitingFree(0)
{
...
{
	if (riid == IID_IAVPushSource)
	{
		return GetInterface((IAVPushSource*)this, ppv);
	}

	if (riid == IID_IAVPushSource2)
	{
		return GetInterface((IAVPushSource2*)this, ppv);
	}

	if (riid == IID_IAVPushSource3)
	{
		return GetInterface((IAVPushSource3*)this, ppv);
	}
...
// GetFrameBuffer
// Provide the caller with a free frame buffer
STDMETHODIMP CCustomAllocator::GetFrameBufferEx(unsigned char** ppBuffer, unsigned long* pSize)
{
	HRESULT hr = S_OK;
...
// to the free list.  If the streaming thread is blocked, waiting for a sample to
// become available on the delivery queue, signal the thread when a new sample is added.
STDMETHODIMP CCustomAllocator::DeliverEx(unsigned char* pBuffer, unsigned long cbSize)
{
	HRESULT hr = S_OK;
	...
	if (pBuffer)
	{
	    ...
	    for (;;)
		{
		    ...
		    if (pSample)
			{
				...
				if (SUCCEEDED(hr))
				{
					pSample->SetActualDataLength(cbSize);
					CAutoLock cObjectLock(this);
					m_lDeliver.AddTail(pSample);
					NotifySample();
					DbgLog((LOG_TRACE, DBG_MEM, TEXT("CCustomAllocator::Deliver(): m_lFree: %d  m_lDeliver: %d"), m_lFree.GetCount(), m_lDeliver.GetCount()));
