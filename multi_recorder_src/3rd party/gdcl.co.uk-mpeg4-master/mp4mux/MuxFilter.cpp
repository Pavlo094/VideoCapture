...
MuxInput::Receive(IMediaSample* pSample)
{
		...
		if(pSample && this->m_index == 0)
		{
			...
			bool bSync = pSample->IsSyncPoint() == S_OK;
			static LONG g_nCounter = 0;
			CHAR pszText[1024] = { 0 };
			sprintf_s(pszText, "%hs: " "m_index %d, nGetTimeResult 0x%08X, nStartTime %I64d, nStopTime %I64d, nDataSize %d flags=%#06x sync=%d (%d)\n", __FUNCTION__, 
				m_index,
				nGetTimeResult, nStartTime / 10000, nStopTime / 10000, 
				nDataSize,
...
{	
	...
	while (count != 0)
	{
		size_t freeMemory = buffer.capacity() - buffer.size();
		size_t length = std::min<long>(freeMemory, count);
...
HRESULT MuxOutput::Flush()
{
    ...
    if (m_bUseIStream)
    {
        ...
        if (pStream == NULL)
        {
			...
			if (!buffer.empty())
			{
				...
				hr = pStream->Seek(liTo, STREAM_SEEK_END, &uliUnused);
				/*if (uliUnused.QuadPart != m_llBytes)
				{
					return E_FAIL;
				}*/
				if (SUCCEEDED(hr))
				{
					ULONG cActual;
					hr = pStream->Write(&buffer[0], buffer.size(), &cActual);
					if (SUCCEEDED(hr) && ((long)cActual != buffer.size()))
					{
						hr = E_FAIL;
					}
				}
	...
	{
		//hr = Replace(m_llBytes, &buffer[0], buffer.size());
	}
	buffer.clear();
	return hr;
}
