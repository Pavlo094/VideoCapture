...
Atom Atom::CreateAtom(DWORD type)
{
    return Atom(this, Length(), type);
}
...
: m_pContainer(pContainer),
  ...
  m_tInterleave(UNITS / 2),
  workerThreadEndRequested(false),
  workerThread(std::bind(&MovieWriter::work, this)),
...
{
	{
		...
		notifyCondition = true;
		workerThreadEndRequested.store(true);
		cv.notify_all();
	}
...
void MovieWriter::work()
{	
	while (!workerThreadEndRequested) 
	{
		{
			std::unique_lock<std::mutex> lck(mtx);
			cv.wait(lck, [this]{return notifyCondition.load();});
			if (workerThreadEndRequested)
				return;	
			notifyCondition = false;
		}
		CheckQueues();
...
MovieWriter::Close(REFERENCE_TIME* pDuration)
{
    ...
    *pDuration = tDur;
    ...
    // create moov atom
    HRESULT hr = S_OK;
    //Atom pmoov = new Atom(m_pContainer, m_pContainer->Length(), DWORD('moov'));
	Atom pmoov(m_pContainer, m_pContainer->Length(), DWORD('moov'));

    // movie header
    // we are using 90khz as the movie timescale, so
    // we may need 64-bits.
    //Atom pmvhd = pmoov->CreateAtom('mvhd');
	Atom pmvhd(pmoov.CreateAtom('mvhd'));
    BYTE b[28*4];
    ZeroMemory(b, (28*4));
    int cHdr;
...
MovieWriter::InsertFTYP(AtomWriter* pFile)
{
    if (!m_bFTYPInserted)
    {
        ...
        Atom pFTYP(pFile, pFile->Length(), DWORD('ftyp'));
        // file type
        BYTE b[8];
		if (bHasOld)
		{
...
void MovieWriter::NotifyCheckQueues()
{
	std::unique_lock<std::mutex> lck(mtx);
	notifyCondition = true;
	cv.notify_all();
...
bool MovieWriter::CheckQueuesSync()
{
	std::unique_lock<std::mutex> lck(mtx);
	return CheckQueues();
}
...
{
	//OutputDebugString(L"CheckQueues\n");
    CAutoLock lock(&m_csWrite);
    if (m_bStopped)
    {
        return false;
...
{
    Atom piods(pmoov.CreateAtom('iods'));
...
TrackWriter::OnEOS()
{
	...
	return m_pMovie->CheckQueuesSync();
}
...
{
    Atom ptrak(patm.CreateAtom('trak'));

    // track header tkhd
    Atom ptkhd(ptrak.CreateAtom('tkhd'));
    BYTE b[24*4];
    ZeroMemory(b, (24*4));
    ...
    Atom pmdia(ptrak.CreateAtom('mdia'));

    // Media Header mdhd
    Atom pmdhd(pmdia.CreateAtom('mdhd'));
    ZeroMemory(b, 9*4);
    ...
    // handler id hdlr
    Atom phdlr(pmdia.CreateAtom('hdlr'));
    ZeroMemory(b, 25);
    ...
    Atom pminf(pmdia.CreateAtom('minf'));

    // media information header vmhd/smhd
    ZeroMemory(b, sizeof(b));
    if (IsVideo())
    {
        Atom pvmhd(pminf.CreateAtom('vmhd'));
        b[3] = 1;
        pvmhd.Append(b, 12);
        pvmhd.Close();
    } else if (IsAudio())
    {
        Atom psmhd(pminf.CreateAtom('smhd'));
        psmhd.Append(b, 8);
        psmhd.Close();
    } else {
        Atom pnmhd(pminf.CreateAtom('nmhd'));
        pnmhd.Append(b, 4);
        pnmhd.Close();
    }

    // dinf/dref -- data reference
    Atom pdinf(pminf.CreateAtom('dinf'));
    Atom pdref(pdinf.CreateAtom('dref'));
    WriteLong(0, b);        // ver/flags
    WriteLong(1, b+4);      // entries
    pdref.Append(b, 8);
    Atom purl(pdref.CreateAtom('url '));
    ...
    Atom pstbl(pminf.CreateAtom('stbl'));

    // Sample description
    // -- contains one descriptor atom mp4v/mp4a/... for each data reference.
    Atom pstsd(pstbl.CreateAtom('stsd'));
    WriteLong(0, b);    // ver/flags
...
{
	if (!data.empty())
		patm.Append(reinterpret_cast<PBYTE>(&data[0]), vectorsizeof(data));
    return S_OK;
}
...
{
	return ReadLong((BYTE*)&data[nEntry]);
}
...
DurationIndex::WriteEDTS(Atom& patm, long scale)
{
    if (m_tStartFirst > 0)
    {
        ...
        Atom pedts(patm.CreateAtom('edts'));
        Atom pelst(pedts.CreateAtom('elst'));
...
DurationIndex::WriteTable(Atom& patm)
{
	...
	if (m_nSamples > 0)
    {
        ...
        Atom pstts(patm.CreateAtom('stts'));
		m_STTS.Write(pstts);
        pstts.Close();
		...
		{
			// write CTTS table
			Atom pctts(patm.CreateAtom('ctts'));
			m_CTTS.Write(pctts);
			pctts.Close();
		}
    }
