...
FOURCCVideoHandler::WriteDescriptor(Atom& patm, int id, int dataref, long scale)
{
	...
	Atom psd = patm.CreateAtom(Swap4Bytes(codec));
	...
	if (m_bMJPG)
	{
		Atom pfiel = psd.CreateAtom(DWORD('fiel'));
		BYTE b[] = {2, 1};
...
WaveHandler::WriteDescriptor(Atom& patm, int id, int dataref, long scale)
{
	...
	if (IsOldIndexFormat())
	{
	    Atom psd = patm.CreateAtom(DWORD('sowt'));
		BYTE b[44];
		ZeroMemory(b, 44);
		WriteShort(dataref, b+6);
	...
	else
	{
	    ...
	    Atom psd = patm.CreateAtom(dwAtom);
		BYTE b[28];
		ZeroMemory(b, 28);
		WriteShort(dataref, b+6);
		WriteShort(2, b+16);
...
void DolbyDigitalHandler::WriteDescriptor(Atom& patm, int id, int dataref, long scale)
{
	...
	Atom pdac3 = psd.CreateAtom('dac3');
	bsw.AppendTo(pdac3);
...
void DolbyDigitalPlusHandler::WriteDescriptor(Atom& patm, int id, int dataref, long scale)
{
	...
	Atom pdec3 = psd.CreateAtom('dec3');
